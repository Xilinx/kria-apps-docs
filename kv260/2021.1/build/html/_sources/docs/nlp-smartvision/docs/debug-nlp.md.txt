...
* Application run without input audio device:

    The application will still run if no audio input device is connected, but only facedetect application will run.
    If you do not get the below message on terminal after launching the application (apx. after 3 seconds), please check the audio device connection.

   `Noise Threshold is set. You can start speaking the keywords now..`
...
2. ensure an active source is connedted at the Pmod LINE IN.
