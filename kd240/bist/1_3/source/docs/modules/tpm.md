...
* tpm2_hash: Gets the hash of a fixed file and verifies it matches the
  expected hash.
* tpm2_pcrread: Verifies PCR 9 and PCR 12 contain matching values for EFI boot validation.
...
```bash
pytest --board kv260 -m tpm			# Run all tests in this module
pytest --board kv260 -k tpm2_getcap		# Run tpm2_getcap test
pytest --board kv260 -k tpm2_hash		# Run tpm2_hash test
```
