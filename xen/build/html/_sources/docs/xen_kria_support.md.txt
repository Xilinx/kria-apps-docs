...
1. Experience and equipments booting a regular PetaLinux on Kria SOM Starter Kit (you can follow [getting started with PetaLinux on KV260](https://www.xilinx.com/products/som/kria/kv260-vision-starter-kit/kv260-getting-started/setting-up-the-sd-card-image.html), but use a wic image from 2023.2 BSP on a minimum 16GB SD Card)
2. A Linux Desktop to create and program partitions
3. PetaLinux or Yocto tool

## Generating Xen Artifacts

Xen artifacts can be generated by Yocto or found in PetaLinux BSP. You can choose to use one of the tools to create Xen artifacts.

### Generating Xen Artifacts in PetaLinux

First, download a 2023.2 or later [Kria SOM Starter Kit BSP](https://xilinx-wiki.atlassian.net/wiki/spaces/A/pages/1641152513/Kria+K26+SOM#PetaLinux-Board-Support-Packages). Extract the PetaLinux project:
...
### Generating Xen Artifacts in Yocto
...
To generate Xen support for KD260, use k26-smk-kd as the ```<machine name>```:
...
Flash the wic image from either [Yocto](#generating-xen-artifacts-in-yocto) or [PetaLinux flow](#generating-xen-artifacts-in-petalinux) to an SD card using Balena Etcher. This will set up the SD card to contain 2 partitions - partition 1 with boot images (accessible by Windows and Linux) that we will discard and partition 2 with PetaLinux rootfs (not accessible by Windows) that will also be used for XEN DOM0.

### Partition 1

If you are using Yocto - partition 1 already contain XEN artifacts as the wic is generated with XEN support. Below step is for PetaLinux Flow only.

PetaLinux only: Open the SD card on a host computer (can be Windows or Linux), go to partition 1, remove the boot images on that partition and copy all the XEN artifacts in ```pre-built/linux/xen``` into partition 1.
...
Locate ```image.fit``` file, it should be in ```/run/media/boot-mmcblk1p1/image.fit``` for 2023.2 KV260 and ```/run/media/boot-sda1/image.fit``` for 2023.2 KR260. If its not there, one can search for it using ```find / -iname "image.fit"```.
...
## PetaLinux as Guest OS

If you would like to run PetaLinux instead of Ubuntu as guest OS, the steps are similar to that of Ubuntu OS as guest OS except for kernel image and root fs artifacts for the perspective OS, as well as the config file. The only 2 artifacts required in this example is in the image folder, file ```Image``` and file ```rootfs.cpio.gz```, copy them into boot partition 1. For partition 3, skip the dd command and leave the partition empty.

Once booted up and in root, copy the two artifacts to partition 3:

```text
cp /run/media/boot-mmcblk1p1/Image /run/media/mmcblk1p3/
cp /run/media/boot-mmcblk1p1/rootfs.cpio.gz /run/media/mmcblk1p3/
```

Use the following as guest0.cfg:

```text
name = "guest0"
kernel = "/run/media/mmcblk1p3/Image"
ramdisk = "/run/media/mmcblk1p3/rootfs.cpio.gz"
...
1. With PetaLinux 2023.2 XEN + Ubuntu 22.04 x07 image - you will see cloud init errors upon booting the guest OS. Some of the peripheral access may not function.
