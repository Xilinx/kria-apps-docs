...
1. PetaLinux 2020.2.2 tools installation
2. Petalinux SOM StarterKit BSP: xilinx-k26-starterkit-2020.2.2-final.bsp
...
![Petalinux rootfs config](media/plnx_rootfs_config.jpg)
...
published at http://petalinux.xilinx.com/sswreleases/rel-v2020.2.2/sdkupdate.

To update the eSDK, extract the petalinux installer, accept the license and
source the tool's settings script.

```bash
petalinux-v2020.2.2-final-installer.run
source settings.sh
```

Remove the existing aarch64 folder.

```bash
rm -rf components/yocto/source/aarch64
```
