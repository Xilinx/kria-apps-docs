...
When using the Kria Starter Kit Linux images you will primarily log in over the USB-UART serial port to access the command line interface. This is required to launch some Kria™ SOM apps. To login and access the command line interface over the USB-UART serial port, choose the appropriate host OS section below:
