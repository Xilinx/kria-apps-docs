...
This document shows how to boot AMD Kria&trade; Starter Kit Linux on KR260 and perform common one-time updates and installations required prior to running any AMD released KR260 application in Linux.
...
1. Flash the Kria Starter Kit Linux image into the SD Card (minimum 16GB).
      ...
      Refer to the [KR260 Getting Started Page](https://www.xilinx.com/products/som/kria/kr260-robotics-starter-kit/kr260-getting-started/setting-up-the-sd-card-image.html) to flash SD card with an Ubuntu 22.04 image.

      You can reuse the same SD card to evaluate all the applications released for Ubuntu 22.04.

2. Set up the hardware. Each application requires a different set of peripherals, below lists the minimum required to boot Linux.
...
1. Sometimes, Ubuntu has a background process ongoing by the time it boots to command prompt and does not yet ask you to change the password. In such cases, doing a snap install might result in a message like following:
...
3. During ```sudo xlnx-config.sysinit``` or ```sudo apt upgrade``` commands, you might see the following errors and exit installation. Rerun the commands and installation should continue and complete.
