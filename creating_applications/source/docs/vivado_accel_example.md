...
There are two ways to start the Vivado project for SOM starterkits. Since the [baremetal example](./baremetal.md) has an example for generating PL design in Vivado using board files, this example will use the base Vivado designs. There are two ways to get the base designs.
