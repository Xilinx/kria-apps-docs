...
The virtual channel with TID 0 is used to feed input data to the accelerator. Only 64 bits from input data are fed to the accelerator. The number of input bits is decided upon by the clock frequency and sampling frequency ratio.
...
- DPU(Deep Learning Processor Unit) is a PL(Programmable Logic) IP optimized for Convolutional neural networks.
...
## PP Pipeline 
PP pipeline (Preprocessing pipeline) RM is used for the input data transformation. This RM performs the resizing, scaling, and format conversion on the input file. It is used along with the DPU RM for the dfx-ml-inference pipeline. It can be used for all the models that have the same preprocessing functions requirement. 

### PP Pipeline Features

This RM performs these operations- 
- Changing NV12 color format to BGR
...
- This pp_pipeline(Pre-processing pipeline) IP is used for the three models - facedetect, refinedet, and ssd.
